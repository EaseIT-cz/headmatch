...
def make_app(tmp_path):
    ...
    app = SimpleNamespace(
        ...
        _offline_prepare_runner=lambda *args, **kwargs: {"prepared": True},
...
def test_run_apo_refine_requires_preset_recording_and_output(tmp_path):
    app = make_app(tmp_path)
    controllers = WorkflowControllers(app)

    controllers.run_apo_refine()
    assert "select an apo preset" in app.status.lower()

    app.apo_preset_var.set("preset.txt")
    controllers.run_apo_refine()
    assert "select a recording wav" in app.status.lower()

    app.apo_refine_recording_var.set("recording.wav")
    app.apo_refine_output_var.set("")
    controllers.run_apo_refine()
    assert "select an output folder" in app.status.lower()


def test_run_apo_refine_success_formats_status(tmp_path, monkeypatch):
    app = make_app(tmp_path)
    app.apo_preset_var.set("preset.txt")
    app.apo_refine_recording_var.set("recording.wav")
    ...
    assert "Refined: L 3.0→1.0 dB, R 4.0→2.0 dB" in app.status


def test_run_apo_import_success_exports_files(tmp_path, monkeypatch):
    app = make_app(tmp_path)
    app.apo_preset_var.set("preset.txt")
    ...
    assert "Imported 2L + 1R filters" in app.status


def test_run_fetch_curve_success_and_failure(tmp_path, monkeypatch):
    app = make_app(tmp_path)
    controllers = WorkflowControllers(app)

    controllers.run_fetch_curve()
    assert "both a URL and output path" in app.status
...
def test_start_online_measurement_validates_and_passes_runner_args(tmp_path):
    app = make_app(tmp_path)
    app.target_csv_var.set(str(tmp_path / "target.csv"))
    app.iteration_mode_var.set("average")
    controllers = WorkflowControllers(app)

    controllers.start_online_measurement()
    result = app.last_task["worker"]()

    assert result["target_path"] == str(tmp_path / "target.csv")
    assert result["output_target"] == "dev1"
    assert result["input_target"] == "dev2"
    assert result["iteration_mode"] == "average"


def test_start_online_measurement_requires_output_dir(tmp_path):
    app = make_app(tmp_path)
...
def test_start_offline_prepare_and_fit_validate_and_schedule(tmp_path, monkeypatch):
    ...
    prepare_result = app.last_task["worker"]()
    assert prepare_result == {"prepared": True}

    controllers.start_offline_fit()
    fit_result = app.last_task["worker"]()
